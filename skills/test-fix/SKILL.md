...
- **`shared-root-cause` mode**:
	...
	1. Once every active member has a verdict, **offer** the sibling-subtask sweep — never run it automatically. List the other subtasks in the Task (post-freshness-check), each with its representative error text, and ask the user which (if any) plausibly share the root cause just diagnosed. Each opted-in sibling re-enters this section from `references/testray-subtask.md`'s **Hydrate Each Member and Freshness Check B** stage (it's already inside the freshness-confirmed Task, so **Freshness Check A** does not reapply). Whichever ticket ends up covering it — folded into the same Task when the root cause matches, or its own new ticket via **Additional step — Group handling in Claim the Failure** when it doesn't — gets its key added to that sibling subtask's `issues` (or the primary subtask's, when folded into the same ticket).
...
- One line reporting the sibling-sweep outcome — offered, declined, or which subtasks were opted in and what came of each — always present even when declined.
